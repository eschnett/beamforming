...
a direct expression: If thread 0 holds value i in register r, then
thread t holds value i + 2*t in its register r.
...
table above. The mapping from C matrix elements to the registers of
the threads in the warp is slightly complicated, so here is the full
mapping. The rows are the threads (0..31), the columns the threads'
registers (0..7):

```
    00 20 08 28 10 30 18 38
    40 60 48 68 50 70 58 78
    80 a0 88 a8 90 b0 98 b8
    c0 e0 c8 e8 d0 f0 d8 f8
    01 21 09 29 11 31 19 39
    41 61 49 69 51 71 59 79
    81 a1 89 a9 91 b1 99 b9
    c1 e1 c9 e9 d1 f1 d9 f9
    02 22 0a 2a 12 32 1a 3a
    42 62 4a 6a 52 72 5a 7a
    82 a2 8a aa 92 b2 9a ba
    c2 e2 ca ea d2 f2 da fa
    03 23 0b 2b 13 33 1b 3b
    43 63 4b 6b 53 73 5b 7b
    83 a3 8b ab 93 b3 9b bb
    c3 e3 cb eb d3 f3 db fb
    04 24 0c 2c 14 34 1c 3c
    44 64 4c 6c 54 74 5c 7c
    84 a4 8c ac 94 b4 9c bc
    c4 e4 cc ec d4 f4 dc fc
    05 25 0d 2d 15 35 1d 3d
    45 65 4d 6d 55 75 5d 7d
    85 a5 8d ad 95 b5 9d bd
    c5 e5 cd ed d5 f5 dd fd
    06 26 0e 2e 16 36 1e 3e
    46 66 4e 6e 56 76 5e 7e
    86 a6 8e ae 96 b6 9e be
    c6 e6 ce ee d6 f6 de fe
    07 27 0f 2f 17 37 1f 3f
    47 67 4f 6f 57 77 5f 7f
    87 a7 8f af 97 b7 9f bf
    c7 e7 cf ef d7 f7 df ff
```

The threads are grouped into 8 groups of 4 threads each. With g=0..7,
t=0..3, the expression becomes: If thread 0 holds value i in register
r, then thread t in group g holds value i + 40*t + g in its register
r.
